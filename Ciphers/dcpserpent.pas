...
class function TCipherSerpent.Algorithm: TCipherAlgorithm;
begin
  Result:= caSerpent;
end;

class function TCipherSerpent.BlockSize: Integer;
begin
  Result := 128;
end;

class function TCipherSerpent.MaxKeySize: integer;
begin
  Result:= 256;
end;

class function TCipherSerpent.SelfTest: boolean;
const
...
var
  ...
  Cipher: TCipherSerpent;
begin
  FillChar(Block, SizeOf(Block), 0);
  Cipher:= TCipherSerpent.Create(@Key1,SizeOf(Key1) * 8);
  ...
  Cipher := TCipherSerpent.Create(@Key2,Sizeof(Key2)*8);
  ...
  Cipher := TCipherSerpent.Create(@Key3,Sizeof(Key3)*8);
...
procedure TCipherSerpent.InitKey(const Key; Size: longword);
var
...
destructor TCipherSerpent.Destroy;
begin
  FillChar(l_key,Sizeof(l_key),0);
  inherited Destroy;
end;

procedure TCipherSerpent.EncryptBlock(const InData; var OutData);
var
  i: integer;
...
procedure TCipherSerpent.DecryptBlock(const InData; var OutData);
var
  i: integer;
  a, b, c, d, e, f, g, h: dword;
