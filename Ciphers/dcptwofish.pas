...
constructor TCipherTwofish.Create(Key: Pointer; KeySize: Integer);
begin
...
destructor TCipherTwofish.Destroy;
begin
...
class function TCipherTwofish.Algorithm: TCipherAlgorithm;
begin
  Result:= caTwofish;
end;

class function TCipherTwofish.BlockSize: Integer;
begin
  Result := 128;
end;

class function TCipherTwofish.MaxKeySize: integer;
begin
  Result:= 256;
end;

class function TCipherTwofish.SelfTest: boolean;
const
...
var
  ...
  Cipher: TCipherTwofish;
begin
  FillChar(Key,Sizeof(Key),0);
  FillChar(Block,Sizeof(Block),0);
  Cipher:= TCipherTwofish.Create(@Key,128);
  for i:= 1 to 49 do
    begin
      Cipher:= TCipherTwofish.Create(@Key,128);
      Move(Block,Key,16);
  ...
  FillChar(Key,Sizeof(Key),0);
  FillChar(Block,Sizeof(Block),0);
  //Cipher := TCipherTwofish.Create(@Key,192);
  for i:= 1 to 49 do
  begin
  ...
  for i:= 1 to 49 do
  begin
    Cipher := TCipherTwofish.Create(@Key,256);
...
procedure TCipherTwofish.InitKey(const Key; Size: longword);
const
...
procedure TCipherTwofish.EncryptBlock(const InData; var OutData);
var
  i: longword;
  t0, t1: DWord;
...
procedure TCipherTwofish.DecryptBlock(const InData; var OutData);
var
  i: integer;
  t0, t1: DWord;
  X: array[0..3] of DWord;
