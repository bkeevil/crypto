...
class function THashHaval.HashSize: integer;
begin
{$IFDEF DIGEST128}
  Result:= 128;
{$ELSE}
...
class function THashHaval.Algorithm: THashAlgorithm;
begin
  Result := haHaval256;
end;

class function THashHaval.SelfTest: boolean;
{$IFDEF PASS3}
  {$IFDEF DIGEST128}
    const
...
procedure THashHaval.Update(const Buffer; Size: longword);
var
  PBuf: ^byte;
begin
...
procedure THashHaval.Final(var Digest);
{$IFNDEF DIGEST256}
  {$IFNDEF DIGEST224}
    var
      temp: dword;
