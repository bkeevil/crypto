...
class function THashMD4.HashSize: integer;
begin
  Result:= 128;
end;

class function THashMD4.Algorithm: THashAlgorithm;
begin
  Result:= haMD4;
end;

class function THashMD4.SelfTest: boolean;
const
...
procedure THashMD4.Update(const Buffer; Size: longword);
var
  PBuf: ^byte;
begin
...
procedure THashMD4.Final(var Digest);
begin
  if not fInitialized then
    raise EHash.Create('Hash not initialized');
  HashBuffer[Index]:= $80;
