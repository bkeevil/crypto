...
class function THashMD5.HashSize: integer;
begin
  Result:= 128;
end;

class function THashMD5.Algorithm: THashAlgorithm;
begin
  Result:= haMD5;
end;

class function THashMD5.SelfTest: boolean;
const
...
procedure THashMD5.Update(const Buffer; Size: longword);
var
  PBuf: ^byte;
begin
...
procedure THashMD5.Final(var Digest);
begin
  if not fInitialized then
    raise EHash.Create('Hash not initialized');
  HashBuffer[Index]:= $80;
