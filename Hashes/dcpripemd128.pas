...
class function THashRipeMD128.HashSize: integer;
begin
  Result:= 128;
end;

class function THashRipeMD128.Algorithm: THashAlgorithm;
begin
  Result:= haRipeMD128;
end;

class function THashRipeMD128.SelfTest: boolean;
const
...
procedure THashRipeMD128.Update(const Buffer; Size: longword);
var
  PBuf: ^byte;
begin
...
procedure THashRipeMD128.Final(var Digest);
begin
  if not fInitialized then
    raise EHash.Create('Hash not initialized');
  HashBuffer[Index]:= $80;
