...
class function THashSHA1.Algorithm: THashAlgorithm;
begin
  Result:= haSHA1;
end;

class function THashSHA1.HashSize: integer;
begin
  Result:= 160;
end;

class function THashSHA1.SelfTest: boolean;
const
...
procedure THashSHA1.Update(const Buffer; Size: longword);
var
  PBuf: ^byte;
begin
...
procedure THashSHA1.Final(var Digest);
begin
  if not fInitialized then
    raise EHash.Create('Hash not initialized');
  HashBuffer[Index]:= $80;
