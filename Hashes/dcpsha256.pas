...
class function THashSHA256.Algorithm: THashAlgorithm;
begin
  Result:= haSHA256;
end;

class function THashSHA256.HashSize: integer;
begin
  Result:= 256;
end;

class function THashSHA256.SelfTest: boolean;
const
...
procedure THashSHA256.Update(const Buffer; Size: longword);
var
  PBuf: ^byte;
begin
...
procedure THashSHA256.Final(var Digest);
begin
  if not fInitialized then
    raise EHash.Create('Hash not initialized');
  HashBuffer[Index]:= $80;
