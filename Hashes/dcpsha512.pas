...
procedure TSHA512Base.Update(const Buffer; Size: longword);
var
  PBuf: ^byte;
begin
...
class function THashSHA384.Algorithm: THashAlgorithm;
begin
  Result:= haSHA384;
end;

class function THashSHA384.HashSize: integer;
begin
  Result:= 384;
end;

class function THashSHA384.SelfTest: boolean;
const
...
procedure THashSHA384.Final(var Digest);
begin
  if not fInitialized then
    raise EHash.Create('Hash not initialized');
  HashBuffer[Index]:= $80;
...
{******************************************************************************}
class function THashSHA512.Algorithm: THashAlgorithm;
begin
  Result:= haSHA512;
end;

class function THashSHA512.HashSize: integer;
begin
  Result:= 512;
end;

class function THashSHA512.SelfTest: boolean;
const
...
var
  TestHash: THashSHA512;
  TestOut: array[0..63] of byte;
begin
  FillChar(TestOut, SizeOf(TestOut), 0);
  TestHash:= THashSHA512.Create;
  TestHash.Init;
  TestHash.UpdateStr('abc');
  TestHash.Final(TestOut);
  Result:= boolean(CompareMem(@TestOut,@Test1Out,Sizeof(Test1Out)));
...
procedure THashSHA512.Final(var Digest);
begin
  if not fInitialized then
    raise EHash.Create('Hash not initialized');
  HashBuffer[Index]:= $80;
